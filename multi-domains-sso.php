...
if( !class_exists("Multidomains_Sso") ):
    ...
    const ACTION_KEY = '__multidomain_action';

    const ACTION_SETUP_CDSSO    = 'multidomain-setup-cdsso';
    const ACTION_CHECK_LOGIN_STATUS = 'multidomain-check-login-status';
    const ACTION_AUTHORIZE_USER = 'multidomain-authorize-user';
    const ACTION_AUTHORIZE_USER_ASYNC = 'multidomain-authorize-user-async';
    const ACTION_PROPAGATE_USER = 'multidomain-propagate-user';
    const SSO_ENDPOINT          = 'md-sso-endpoint';
    ...
    public function add_propagation_script() {
        ...
        <script <?php echo $this->_async ? "async='true'" : ""; ?>  type="text/javascript">
            function multidomain_do_redirect() { window.location = "<?php echo $redirect_to ?>"; }
            setTimeout(multidomain_do_redirect, 5000);
        </script>
    ...
    private function _add_auth_script_sync(){
        ...
        $url = add_query_arg( 'md_action', self::ACTION_SETUP_CDSSO, $this->_get_sso_endpoint_url() );
    ...
    private function _add_auth_script_async(){
        ...
        $url = add_query_arg( array(
                'md_action' => self::ACTION_CHECK_LOGIN_STATUS,
    ...
    function _flush_rewrite_rules(){
        $key =  multi_domain::FLUSHED_REWRITE_RULES . get_current_blog_id();
    ...
    function dispatch_ajax_request(){
        ...
        if( isset( $_REQUEST["md_action"] ) ){
            $action = $_REQUEST["md_action"];
        }elseif( isset( $_REQUEST["q"] ) ){
            $q = $_REQUEST["q"];
            $parsed = parse_url($q );

            if( isset( $parsed['query'] ) && strpos($parsed['query'], "md_action") !== false ){ // if query is set
                $action = str_replace("md_action=", "", $parsed['query']);
            }else{
                $prefix = "?md_action=";
                $pos = strpos( $q, $prefix );
        ...
        if( !empty( $action ) ){
            $method = str_replace(array("multidomain-", "-"), array("", "_"), $action);

            if( method_exists("Multidomains_Sso", $method) )
    ...
    function check_login_status(){
        ...
        <?php
        $args = array(
            "md_action" => self::ACTION_AUTHORIZE_USER_ASYNC,
